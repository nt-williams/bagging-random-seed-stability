...
png("paper/plots/nnet-phase.png", width = 3, height = 2.5, units = "in", res = 600)
...
# polygon(
#   x = c(epsilon, rev(epsilon)),
#   y = c(rep(par("usr")[4], length(epsilon)), rev(bound(epsilon))),
#   col = rgb(1, 0, 0, 0.15), border = NA
# )
# text(
#   x = 0.5, y = 0.65,
#   labels = expression(paste("Not (", epsilon, ",", delta, ")-stable")),
#   cex = 0.8
# )
dev.off()